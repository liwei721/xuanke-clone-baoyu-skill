...
### AI illustration placeholders (GLM CogView, default `cogview-3-flash`)

When the markdown uses **generated image** placeholders instead of real files, `baoyu-md` resolves them via the Z.AI **images/generations** API before embedding images in HTML.

**Syntax** (pick one):

1. **Shorthand** (default model `cogview-3-flash`): `![caption](gen:your prompt text)` — do not put `)` inside the prompt; use the URL form below for complex prompts.
2. **Explicit model**: `![caption](gen://cogview-3-flash/your%20prompt%20here)` — path after the first `/` is the prompt (URL-encoded as needed).

**Environment**: `ZAI_API_KEY` or `BIGMODEL_API_KEY` (required). Optional: `BAOYU_GEN_IMAGE_MODEL` (overrides default model for shorthand only), `ZAI_BASE_URL` / `BIGMODEL_BASE_URL`, `BAOYU_MD_DISABLE_GEN=1` to block generation.

This applies to `scripts/main.ts` and any skill that uses the shared `baoyu-md` image pipeline (for example `baoyu-post-to-wechat` markdown → HTML).
