...
**贴图（newspic）→ 草稿箱（API）**：与 `news` 相同走 `draft/add`，设置 `--type newspic`。正文中每张图会走素材上传并填入 `image_info.image_list`。可直接给 **仅含图片的目录**（按文件名排序上传），无需手写 markdown：

```bash
${BUN_X} {baseDir}/scripts/wechat-api.ts ./my-album-folder --type newspic --title "相册标题" --theme default
```

多图文章也可用 markdown 内联 `![](...)` 引用多张本地图后同样 `--type newspic`。
...
- `article_type`: `news` (default, 文章) or `newspic`（贴图 / 多图笔记，对应公众平台「贴图」）
